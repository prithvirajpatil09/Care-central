...
def hospitaldetailview(request,hospital_id):
    ...
    facility = HospitalFacilities.objects.filter(hospital=hospital_id)
    doctors = Doctor.objects.filter(workplace = hospital_id)
    events = Event.objects.filter(hospitalid = hospital_id)
    context = {'hospital':hospital, 'facility' : facility, 'doctors' : doctors, 'events' : events}
    return render(request,'MDTouch/hospitaldetail.html',context)
...
class TestList(generics.ListCreateAPIView):
    ...
    filter_backends = (DjangoFilterBackend,filters.OrderingFilter,)
    filter_fields = ('name','description','released','centre','patient','doctor','date')
    ...
    if len(query) is not 0 and not (query == '' or query == ' ' or query == '   '):
        ...
        for a in symptom:
            if a in query:
                ...
                result.append(0)
        result = disease.searchdisease(symptom,result)
        context = {
            'symptom' : symptom,
            'result' : result
        }
        return render(request,'MDTouch/disease-search.html',context)
    else:
